...
const brutalFooterCSS = `
/* ─── BRUTAL FOOTER ────────────────────────────────────────────── */
.brutal-footer {
  border-top: 4px solid var(--main-color) !important;
}
.brutal-footer-links li {
  margin-bottom: 1.5rem;
}
.brutal-footer-links a {
...
.brutal-footer-links a:hover {
  color: var(--light-color);
  transform: translateX(10px);
}
.brutal-footer-links a:hover::after {
  transform: scaleX(1);
...
if (!cssContent.includes('BRUTAL FOOTER')) {
    cssContent += brutalFooterCSS;
...
const footerRegex = /<!--[\\s\\S]*?FOOTER[\\s\\S]*?-->[\\s\\S]*?<\\/footer>/;
